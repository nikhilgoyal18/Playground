...
def generate_answer(state: SearchState) -> dict:
    ...
    # Group chunks by article (don't dedup) to preserve rich content per source
    articles = {}
    for doc, meta in zip(state["docs"], state["metas"]):
        key = (meta.get("source_type"), meta.get("date"), meta.get("author"), meta.get("title"))
        if key not in articles:
            articles[key] = {"meta": meta, "chunks": []}
        articles[key]["chunks"].append(doc)

    # Build context blocks: one header per unique article, all chunks under it
    context_blocks = []
    for i, (article_key, entry) in enumerate(articles.items(), start=1):
        meta = entry["meta"]
        ...
        # Combine all chunks from this article with newlines
        article_content = "\n".join(entry["chunks"])
    ...
    # Reconstruct docs/metas for state (article-grouped)
    final_docs = []
    final_metas = []
    for article_key, entry in articles.items():
        final_docs.extend(entry["chunks"])
        final_metas.extend([entry["meta"]] * len(entry["chunks"]))
