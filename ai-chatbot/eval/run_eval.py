...
def print_table(results: list):
    ...
    for r in results:
        ...
        all_ok = r["path_correct"] and r["judge_ok"] and r["has_answer"]
        if r["errors"]:
            status = "ERR"
        elif all_ok:
            status = "PASS"
    ...
    failures = [r for r in results if not (r.get("path_correct") and r.get("judge_ok") and r.get("has_answer"))]
    if failures:
...
def main():
    ...
    passed = sum(1 for r in results if r.get("path_correct") and r.get("judge_ok") and r.get("has_answer"))
